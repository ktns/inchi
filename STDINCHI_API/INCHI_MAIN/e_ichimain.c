...
/*            e_bEnableCmdlineOption( szInchiCmdLine, "Inchi2Struct", 1 );*/
            nRet3 = GetStructFromStdINCHI( &inpInchi, &outStructure );
/*            e_bEnableCmdlineOption( szInchiCmdLine, "Inchi2Struct", 0 );*/
            
            FreeStdINCHI( pOut ); /* we do not need the original InChI anymore */
            ...
            if ( nRet3 == inchi_Ret_OKAY || nRet3 == inchi_Ret_WARNING ) {
                ...
                nRet4 = GetStdINCHI( &inchi_inp2, &inchi_out2 );
                ...
                inchi_ios_eprint( log_stream, "GetStdStructFromINCHI failed (%s) %s\n", outStructure.szMessage? outStructure.szMessage:"???", pStrLogId );
            }
        ...
        if ( nRet1 == inchi_Ret_OKAY || nRet1 == inchi_Ret_WARNING ) {
            ...
            InchiInpData idat;
            /* setup input for Get_std_inchi_Input_FromAuxInfo */
            idat.pInp = pInp2;
            pInp2->szOptions = NULL; /* not needed */
            
            /* Make InChI input out of AuxInfo */
            nRet2 = Get_std_inchi_Input_FromAuxInfo( szInchiAuxInfo, bDoNotAddH, &idat );
        ...
        if ( pOut->szInChI && pOut->szInChI[0] ) {
            if (ip->bINChIOutputOptions & INCHI_OUT_SDFILE_ONLY ) {
                ...
                int bAuxOrKey = bAuxInfo || ( ip->bCalcInChIKey == 1 );
                ...
                if ( bAuxInfo ) {
                    inchi_ios_print(out_stream, "%s%s",pOut->szAuxInfo, ip->bCalcInChIKey? pTAB : pLF); 
                }
                ...
                {                      
					...
					ik_ret = GetStdINCHIKeyFromStdINCHI(pOut->szInChI, ik_string);
#else
					/* to be filled */
					ik_ret = INCHIKEY_UNKNOWN_ERROR;
#endif
                    if (ik_ret==INCHIKEY_OK)    
                        inchi_ios_print(out_stream, "InChIKey=%-s\n",ik_string);
                    else    
                    {
                        ...
                        case INCHIKEY_ERROR_IN_FLAG_CHAR:
                                inchi_ios_eprint(log_stream, "(detected error in flag character)\n");
                                break;
