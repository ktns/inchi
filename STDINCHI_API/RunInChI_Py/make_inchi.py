...
def process_sdf_record(insdf,irec,r,fw,fl,iopts,baux,getinchi,freeinchi,getikey,ikey,inchilogmess, nerr):
...
if __name__ == "__main__":
	...
	print "This Python demo program reads SD file and calls standard InChI library"
	print "(libstdinchi.dll/libstdinchi.so.1) functions to generate "
	print "standard InChI strings and standard InChIKey codes."
...
""")
	...
	parser.add_option('-k', '--key', dest='key', 
			help='calculate standard InChIKey', default=False, action='store_true')
	...
	opsys = sys.platform
	if (opsys[:3]=='win'):
		libname = 'libstdinchi.dll'
	else:
		if (opsys[:5]=='linux'):
			libname = '/usr/lib/libstdinchi.so.1'
	try:
		libinchi = cdll.LoadLibrary(libname)
		getinchi = libinchi.GetStdINCHI
		freeinchi = libinchi.FreeStdINCHI
		getikey = libinchi.GetStdINCHIKeyFromStdINCHI
	except:
		print '** ERROR ** Could not access standard InChI library', libname
		sys.exit()
	inchilogmess = ""
	timing = time.clock()
	i = -1
